...
void main() {
  group('chat', () {
    ...
    test('updateTitle', () async {
      ...
      expect(chat2!.title, 'good title');

      final chat3 = await server.loadChat(chat2.id);
      expect(chat3?.title, 'good title');
    });
    ...
    test('chat(sendMessageWithoutStream)のテスト', () async {
      ...
      final message1 = await server.sendMessageWithoutStream(chat1, 'prompt1',
          dateTime: now);
      ...
      final message2 = await server.sendMessageWithoutStream(chat1, 'prompt2',
          dateTime: now);
